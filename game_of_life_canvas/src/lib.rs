...
impl Universe {
    ...
    // 8. we'll need to add some more getter functions for a universe's width,
    // height, and pointer to its cells array.
...
impl fmt::Display for Universe {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        for line in self.cells.as_slice().chunks(self.width as usize) {
            for &cell in line { // ⬜ U+2B1C □ U+25A1   ⬛ U+2B1B ■U+25A0
